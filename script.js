...
const smore = document.getElementById("smore");
...
function createCard(image, number, name, rank, description) {
	...
	const numbeer = document.createElement("h2");
	numbeer.textContent = number;

	const namee = document.createElement("h1");
	namee.textContent = name;

	const rankk = document.createElement("h2");
	rankk.textContent = rank;

	const imagee = document.createElement("img");
	imagee.setAttribute("src", image);
	imagee.setAttribute("alt", number);
	...
	cardTop.appendChild(numbeer);
	cardTop.appendChild(namee);
	cardTop.appendChild(rankk);

	card.appendChild(imagee);
	card.appendChild(descriptionn);
...
ss.addEventListener("click", () => {
	...
	for (let i = 0; i < cards.length; i++) {
		...
		if (card.rank.startsWith("SS")) {
			cartesFiltrer.push(card);
		}
	}

	resetMain();
	displayCards(cartesFiltrer);
});
...
search.addEventListener("input", () => {
	...
	for (let i = 0; i < cards.length; i++) {
		...
		if (card.name.includes(search.value)) {
			cartesFiltrer.push(card);
		}
	}
